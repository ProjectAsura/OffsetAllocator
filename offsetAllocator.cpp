...
#ifdef DEBUG_VERBOSE
#include <cstdio>
#define DEBUG_VERBOSE_CODE(x)   x
#else
#define DEBUG_VERBOSE_CODE(x)
#endif
...
: m_size            (other.m_size)
, m_maxAllocsCount  (other.m_maxAllocsCount)
...
Allocation Allocator::allocate(uint32_t size)
{
    ...
    DEBUG_VERBOSE_CODE(printf_s("Free storage: %u (-%u) (allocate)\n", m_freeStorage, nodeTotalSize));
...
void Allocator::free(Allocation allocation)
{
    ...
    DEBUG_VERBOSE_CODE(printf_s("Putting node %u into freelist[%u] (free)\n", nodeIndex, m_freeOffset + 1));
...
uint32_t Allocator::insertNodeIntoBin(uint32_t size, uint32_t dataOffset)
{
    ...
    DEBUG_VERBOSE_CODE(printf_s("Getting node %u from freelist[%u]\n", nodeIndex, m_freeOffset + 1));
    ...
    m_freeStorage += size;
    DEBUG_VERBOSE_CODE(printf_s("Free storage: %u (+%u) (insertNodeIntoBin)\n", m_freeStorage, size));
...
void Allocator::removeNodeFromBin(uint32_t nodeIndex)
{
    ...
    DEBUG_VERBOSE_CODE(printf_s("Putting node %u into freelist[%u] (removeNodeFromBin)\n", nodeIndex, m_freeOffset + 1));
    m_freeNodes[++m_freeOffset] = nodeIndex;

    m_freeStorage -= node.dataSize;
    DEBUG_VERBOSE_CODE(printf_s("Free storage: %u (-%u) (removeNodeFromBin)\n", m_freeStorage, node.dataSize));
}
...
StorageReport Allocator::storageReport() const
{
    ...
    if (m_freeOffset >= 0)
    {
        ...
        if (m_usedBinsTop)
        {
            uint32_t topBinIndex  = 31 - std::countl_zero(m_usedBinsTop);
            uint32_t leafBinIndex = 31 - std::countl_zero(uint32_t(m_usedBins[topBinIndex]));
