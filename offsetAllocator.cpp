...
#ifdef DEBUG_VERBOSE
    printf_s("Free storage: %u (-%u) (allocate)\n", m_freeStorage, nodeTotalSize);
#endif
...
#ifdef DEBUG_VERBOSE
    printf_s("Putting node %u into freelist[%u] (free)\n", nodeIndex, m_freeOffset + 1);
#endif
...
uint32_t Allocator::insertNodeIntoBin(uint32_t size, uint32_t dataOffset)
{
...
#ifdef DEBUG_VERBOSE
    printf_s("Getting node %u from freelist[%u]\n", nodeIndex, m_freeOffset + 1);
#endif
    ...
    m_freeStorage += size;
#ifdef DEBUG_VERBOSE
    printf_s("Free storage: %u (+%u) (insertNodeIntoBin)\n", m_freeStorage, size);
#endif
...
#ifdef DEBUG_VERBOSE
    printf_s("Putting node %u into freelist[%u] (removeNodeFromBin)\n", nodeIndex, m_freeOffset + 1);
#endif
    m_freeNodes[++m_freeOffset] = nodeIndex;

    m_freeStorage -= node.dataSize;
#ifdef DEBUG_VERBOSE
    printf_s("Free storage: %u (-%u) (removeNodeFromBin)\n", m_freeStorage, node.dataSize);
#endif
}
