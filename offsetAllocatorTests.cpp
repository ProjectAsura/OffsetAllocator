...
TEST(allocate, offsetAllocator)
{
    ...
    SECTION("reuse complex")
    {
        ...
        EXPECT_TRUE(c.offset == 1024 + 3456);

        OffsetAllocator::Allocation d = allocator.allocate(456);
        EXPECT_TRUE(d.offset == 0);

        OffsetAllocator::Allocation e = allocator.allocate(512);
        EXPECT_TRUE(e.offset == 456);

        OffsetAllocator::StorageReport report = allocator.storageReport();
        EXPECT_TRUE(report.totalFreeSpace == 1024 * 1024 * 256 - 3456 - 2345 - 456 - 512);
        EXPECT_TRUE(report.largestFreeRegion != report.totalFreeSpace);
    ...
    SECTION("zero fragmentation")
    {
        ...
        for (uint32_t i = 0; i < 256; i++)
        {
            ...
            EXPECT_TRUE(allocations[i].offset == i * 1024 * 1024);
        }

        OffsetAllocator::StorageReport report = allocator.storageReport();
        EXPECT_TRUE(report.totalFreeSpace == 0);
        EXPECT_TRUE(report.largestFreeRegion == 0);
